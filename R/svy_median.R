...
#' @param vars A variable to calculate median from.
...
svy_median <- function(design, vars, group = NULL,  group_key_sep = " -/- ", na_rm = TRUE, vartype = "ci", level = 0.95, ak = TRUE, ak_overall_sep = " @/@ ", ak_main_sep = " -/- ", ak_var_to_value_sep = " %/% ", ...){
  ...
  make_median <- function(design, var, group, group_key, group_key_sep, na_rm, vartype, level, ...){
    ...
    to_return <- srvyr::summarize(
      to_return,
      "stat" := srvyr::survey_median(!!rlang::sym(var), vartype = vartype, level = level, ...),
      "stat_unw" := srvyr::unweighted(stats::median(!!rlang::sym(var))),
