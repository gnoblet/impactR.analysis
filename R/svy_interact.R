...
#' @param col A column to calculate interactions from.
...
#'
#' @export
svy_interact <- function(design, col, group = NULL, unnest_interaction = TRUE, na_rm = TRUE, stat_name = "mean", vartype = "ci", level = 0.95, deff = FALSE, ...){
  ...
  to_return <- srvyr::group_by(design, srvyr::across({{ group }}), srvyr::interact(interaction = srvyr::across({{ interact_cols }})))
  ...
  to_return <- srvyr::summarize(
    to_return,
    "{stat_name}" := srvyr::survey_mean({{ col }}, vartype = vartype, level = level, deff = deff, ...),
