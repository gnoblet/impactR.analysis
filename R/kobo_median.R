...
#' @param vars Variables to calculate median from.
...
kobo_median <- function(design, vars, survey, group = NULL, group_key_sep = " -/- ", label_survey = TRUE, na_rm = TRUE, vartype = "ci", level = 0.95){
  ...
  median <- svy_median(design, vars, group =  group, vartype = vartype, level = level, na_rm = na_rm)
