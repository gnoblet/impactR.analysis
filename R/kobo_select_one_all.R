...
kobo_select_one_all <- function(design, survey, choices = NULL, group = NULL, group_key_sep = " -/- ", label_survey = TRUE, na_rm = TRUE, vartype = "ci", level = 0.95){
  ...
  if (length(select_ones) == 0) {
    ...
    rlang::warn("There are no vars of type select_one which are not grouping columns. An empty tibble is returned.")
    return (dplyr::tibble())

  }

  proportions <- kobo_select_one(design = design, vars = select_ones, survey = survey, choices = choices, group =  group, label_survey = label_survey, na_rm = na_rm, vartype = vartype, level = level)
