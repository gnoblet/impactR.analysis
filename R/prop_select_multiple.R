...
#' @param var A qupted variable to calculate proportion from.
...
#'
#' @export
prop_select_multiple <- function(design, var, survey, choices, choices_sep = "_", group = NULL, group_key_sep = " ~/~ ", label_survey = TRUE, label_choices = TRUE, na_rm = TRUE, vartype = "ci", level = 0.95){

  # Check
  if_not_in_stop(design$variables, var, "design", "var")
  ...
  if (!(var %in% select_multiples)) rlang::abort(
    c("Variable is not a select_multiple in survey.",
      "i" = glue::glue("You may check that column '{var}' is a `select_multiple` question. Maybe verify that the survey sheet is the right and most updated version.")
    ))
  ...
  if (length(select_multiple_child_in_design) == 0) rlang::abort(
    c("No child column.",
      ...
      "i" = glue::glue("You may check that column '{var}' is a `select_multiple` question. You may also check that the provided separator for choices is the right one (param 'choices_sep' default to '_'). Finally, you can verify that the survey sheet is the right and most updated version.")
    ))
  ...
  if (length(select_multiple_child_not_in_design) > 0) rlang::warn(glue::glue(paste0("The following child columns of '{var}' does not exist in `design`, there are removed for the proportion calculation: ", paste(
